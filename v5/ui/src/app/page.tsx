...
export default function LandingAndDashboard() {
  ...
  return (
    <div className="min-h-screen bg-transparent text-[var(--color-text-primary)] font-sans selection:bg-[#00D4FF] selection:text-[#050816]">
      ...
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${isNavScrolled ? 'bg-[#050816]/60 backdrop-blur-2xl border-b border-white/5 py-4 shadow-xl' : 'bg-transparent py-6'}`}>
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <span className="font-bold text-2xl tracking-tight font-display text-white group-hover:text-gray-300 transition-colors">DermaScan AI <span className="text-[#00D4FF] text-lg font-medium">V5</span></span>
          </Link>
          ...
          <div className="max-w-2xl">
            ...
            >
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#111827]/80 backdrop-blur-md border border-white/10 mb-8 shadow-lg">
                ...
                <span className="text-xs font-semibold text-[#00D4FF] tracking-wide uppercase">Dual-Head Deep Learning</span>
              </div>
              ...
              <p className="text-lg md:text-xl text-[var(--color-text-secondary)] font-light leading-relaxed mb-10 max-w-xl backdrop-blur-sm bg-[#050816]/10 p-2 rounded-lg">
                Upload a dermoscopic image and let DermaScan AI V5 perform dual-head analysis predicting both specific diagnosis and general etiology from raw image pixels alone — powered by explainable AI and clinical decision support.
              </p>
          ...
          >
            ...
            <div className="relative z-10 flex flex-col items-center pointer-events-none">
              ...
              <div className="mt-6 flex items-center gap-3 bg-[#111827]/80 backdrop-blur-md px-4 py-2 rounded-lg border border-white/10 shadow-[0_10px_20px_rgba(0,0,0,0.5)]">
                ...
                <span className="text-sm font-medium tracking-wide text-white">Dual-Head Neural Network</span>
              </div>
            </div>
          </motion.div>
        </motion.div>
      ...
      {/* ✅ Medical Dashboard UI Section — V5 Enhanced */}
      <section id="dashboard-scanner" className="scroll-mt-24 py-24 bg-[#0B1120]/40 backdrop-blur-lg border-t border-b border-white/5 relative z-20 shadow-[0_0_50px_rgba(0,0,0,0.5)]">
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          ...
          <div className="text-center mb-16">
            ...
            <p className="text-[var(--color-text-secondary)]">Secure, real-time dual-head image analysis pipeline.</p>
          </div>
          ...
          <div className="grid lg:grid-cols-12 gap-8">
            ...
            >
              <div className="premium-card p-6 border-t border-l border-white/20 h-full flex flex-col justify-between">
                ...
                <div className="bg-[#111827]/80 backdrop-blur-md border border-white/10 p-4 rounded-xl mb-6 shadow-inner">
                  <h4 className="text-sm font-bold text-[#00D4FF] mb-2 flex items-center gap-2">
                    <Info size={16} /> Scanning Directions
                  </h4>
              ...
              <div className="premium-card p-3 h-auto min-h-[600px] flex relative overflow-hidden bg-[#050816]/60 border-t border-l border-white/20 shadow-2xl">
                {previewUrl ? (
                  <div className="w-full h-full flex flex-col md:flex-row gap-4 transition-all duration-200 ease-in-out">
                    {/* Visual Canvas (Left) */}
                    <div className={`relative rounded-xl overflow-hidden flex flex-col items-center justify-center bg-black/60 shadow-inner transition-all duration-200 ease-in-out min-h-[300px] gap-4 p-4 py-8 overflow-y-auto ${result && !result.error && !isScanning ? 'flex-1' : 'w-full h-full'}`}>
                      {(!result || isScanning || result.error) ? (
                        ...
                        <>
                          <div className="flex flex-col items-center gap-2">
                            <span className="text-xs text-gray-400 font-semibold uppercase tracking-wider">Model Input (380x380)</span>
                            {result.cropped_image ? (
                              <img src={result.cropped_image} alt="Cropped Lesion" className="w-full max-w-[300px] object-contain rounded-lg shadow-md border border-white/10" />
                            ) : (
                              <img src={previewUrl} alt="Preview" className="w-full max-w-[300px] object-contain rounded-lg shadow-md border border-white/10" />
                            )}
                          </div>
                          
                          <div className="flex flex-col items-center gap-2 mt-4">
                            <span className="text-xs text-[#00D4FF] font-semibold uppercase tracking-wider">Grad-CAM Activation</span>
                            <AnimatePresence>
                              {result.heatmap && (
                                <motion.img 
                                  initial={{ opacity: 0 }}
                    ...
                    <AnimatePresence>
                      {result && !result.error && !isScanning && (
                        ...
                        >
                          ...
                          {/* ✅ V5: Etiology Category Bars */}
                          <h4 className="text-xs font-semibold text-white mt-6 mb-4 flex items-center gap-2 drop-shadow">
                            <Layers size={14} className="text-[#00D4FF]" /> 
                            Etiology Safety Net (4 Families)
                          </h4>
                          
                          <div className="space-y-3">
                            {Object.entries(result.all_etiology || {})
                              .sort(([, a], [, b]) => (b as number) - (a as number))
                              .map(([className, prob], idx) => {
                                const percentage = ((prob as number) * 100).toFixed(1);
                                const isTop = idx === 0;
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center text-gray-400 bg-black/20 rounded-lg shadow-inner border border-white/5">
                    ...
                    <p className="text-sm mt-2 max-w-xs text-center">Upload a dermoscopic image to initiate the dual-head diagnostic pipeline.</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="why-grid">
          <div className="glass p-10 bg-[#111827]/50 border border-white/10 rounded-2xl shadow-2xl">
            ...
            <div className="flex flex-col gap-4">
              {[
                { title: "Dual-Head Inference", hiddenText: "Simultaneously predicts the specific differential diagnosis (10 classes) and broader etiology family (4 classes) using parallel neural network heads." },
                ...
                { title: "Etiology Safety Net", hiddenText: "Even if the specific diagnosis is slightly off, the model accurately predicts the overarching lesion family (Melanocytic, Keratinocytic, Vascular, Inflammatory) to ensure dangerous cancers aren't miscategorized." },
                { title: "V5 Risk Calculator", hiddenText: "Aggregates the 14 probability logits into a single, clinically actionable four-tier risk level (from Benign to Malignant) with color-coded medical recommendations." },
          ...
          <motion.div 
            ...
            onMouseLeave={tilt1.handleMouseLeave}
          >
            <div className="badge">Model Card — V5 Engine</div>
            <h3 className="text-white font-display">EfficientNet-B4, Dual-Head</h3>
            <p>Compound-scaled convolutional backbone, transfer-learned on ISIC 2024 dermoscopy data. Outputs 10 specific diagnostic categories and 4 etiology categories. Pure image-based inference.</p>
            <div className="flex flex-col gap-6 mt-6">
              ...
              <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/10">
                <div className="flex flex-col gap-1">
                  <span className="text-gray-400 text-xs uppercase tracking-wider font-semibold">Output Neurons</span>
                  <span className="text-white font-mono text-lg font-bold">14 (10 + 4)</span>
                </div>
                <div className="flex flex-col gap-1">
      ...
      <section id="technology" className="py-24 max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        <div className="mb-16">
          ...
          <p className="text-gray-300 max-w-2xl">A single forward pass through a branching neural network — raw image features are split into two parallel classifiers to compute context-aware predictions.</p>
        </div>
        ...
        <div className="glass bg-[#111827]/50 border border-white/10 rounded-2xl shadow-2xl mb-24" style={{padding: '36px 28px'}}>
          <div className="pipeline" id="pipeline">
            {[
              ...
              { arrow: true },
              { 
                icon: Network, title: "Dual-Head Split", sub: "PARALLEL CLASSIFIERS", 
                modal: <><p className="mb-3 text-gray-300"><strong>Overview:</strong> The decision-making center splits into two parallel brains to weigh the visual evidence simultaneously.</p><p className="text-gray-400"><strong>Technical Detail:</strong> The CNN features branch into two separate fully connected neural network stacks. One maps to 10 specific diagnoses, while the other acts as a safety-net mapping to 4 broad etiology families.</p></>
              },
              { arrow: true },
              { 
              ...
              { arrow: true },
              { 
                icon: CheckCircle2, title: "10 DDx + 4 Etiology Classes", sub: "+ CLINICAL MAPPING", 
                modal: <><p className="mb-3 text-gray-300"><strong>Overview:</strong> The final output placing the lesion into 10 specific differential diagnoses and 4 overarching etiology families.</p><p className="text-gray-400"><strong>Technical Detail:</strong> The two heads output separate logits. Head 1 (10 classes) identifies specific ISIC 2024 classes. Head 2 (4 classes) groups them into Melanocytic, Keratinocytic, Vascular, or Inflammatory families.</p></>
              }
            ].map((step, idx) => {
              if (step.arrow) {
                return (
        ...
        <div className="flex flex-col gap-10">
          <div className="max-w-3xl">
            ...
            <p className="text-gray-300 leading-relaxed mb-6 text-lg">
              Our model doesn&apos;t just output a prediction. Using Gradient-weighted Class Activation Mapping (Grad-CAM) on the <code className="text-[#00D4FF] bg-white/5 px-1.5 py-0.5 rounded text-sm">top_conv</code> layer, DermaScan AI V5 highlights the exact pixel regions that influenced the model&apos;s decision.
            </p>
            <p className="text-gray-400 leading-relaxed mb-10">
              The dual-head architecture ensures a clinical safety net by correlating the specific diagnosis prediction against the general etiology prediction, flagging any contradictory or uncertain cases for dermatologist review.
            </p>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <div className="glass foot-card bg-[#111827]/80 backdrop-blur-3xl border border-white/10 shadow-2xl rounded-3xl">
            ...
            <div className="foot-bottom">
              <div>© 2026 DermaScan AI V5 · Research Prototype</div>
